  ...
  <div class="min-h-screen flex flex-col flex-grow bg-meal-light font-sans pb-8">
    ...
    <header class="bg-meal-primary text-white p-4 shadow-md mb-8">
      <div class="container mx-auto flex items-center justify-between">
        <div class="flex items-center space-x-2">
          ...
          <h1 class="text-2xl font-header font-bold">Dashboard</h1>
        </div>
    ...
    <div class="container mx-auto px-4">
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        <div class="bg-white rounded-xl shadow-meal p-6">
          <div class="flex items-center justify-between mb-4">
            <h3 class="text-lg font-header font-bold text-meal-gray-dark">Mahlzeiten</h3>
            <div class="p-2 bg-meal-light rounded-lg">
              <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 text-meal-primary" fill="none" viewBox="0 0 24 24"
                   stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
          ...
          <p class="text-3xl font-bold text-meal-gray-dark mb-1">{{ stats.meals }}</p>
        ...
        <div class="bg-white rounded-xl shadow-meal p-6">
          <div class="flex items-center justify-between mb-4">
            <h3 class="text-lg font-header font-bold text-meal-gray-dark">Ausgaben</h3>
            <div class="p-2 bg-meal-light rounded-lg">
              <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 text-meal-accent" fill="none" viewBox="0 0 24 24"
                   stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
          ...
          <p class="text-3xl font-bold text-meal-gray-dark mb-1">{{ formatCurrency(stats.totalSpent) }}</p>
          <div class="flex items-center text-sm">
        ...
        <div class="bg-white rounded-xl shadow-meal p-6">
          <div class="flex items-center justify-between mb-4">
            <h3 class="text-lg font-header font-bold text-meal-gray-dark">Benutzer</h3>
            <div class="p-2 bg-meal-light rounded-lg">
              <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 text-meal-secondary" fill="none"
          ...
          <p class="text-3xl font-bold text-meal-gray-dark mb-1">{{ stats.users }}</p>
          <div class="flex items-center text-sm">
        ...
        <div class="bg-white rounded-xl shadow-meal p-6">
          <div class="flex items-center justify-between mb-4">
            <h3 class="text-lg font-header font-bold text-meal-gray-dark">Offene Schulden</h3>
            <div class="p-2 bg-meal-light rounded-lg">
              <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 text-meal-error" fill="none" viewBox="0 0 24 24"
                   stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
          ...
          <p class="text-3xl font-bold text-meal-gray-dark mb-1">{{ formatCurrency(stats.totalDebt) }}</p>
          <div class="flex items-center text-sm">
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
        ...
        <!-- Ausgaben pro Person -->
        <div class="bg-white rounded-xl shadow-meal p-6">
          <h3 class="text-lg font-header font-bold text-meal-gray-dark mb-4">Ausgaben pro Person</h3>
          <div class="h-80 flex items-center justify-center">
            ...
            <div class="relative w-64 h-64">
              ...
              <div
                  class="absolute w-32 h-32 bg-white rounded-full top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2"></div>
            </div>

            <!-- Legende -->
            <div class="ml-6 space-y-2">
              <div v-for="user in userExpenseData" :key="user.id" class="flex items-center">
                <div class="w-4 h-4 rounded mr-2" :style="{ backgroundColor: getUserColor(user.id) }"></div>
                <span class="text-sm text-meal-gray-dark">{{ user.name }} ({{ formatPercentage(user.percentage) }})</span>
              </div>
            </div>
          </div>
        </div>
      ...
      <div class="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-8">
        <!-- Letzte Aktivitäten -->
        <div class="bg-white rounded-xl shadow-meal p-6 lg:col-span-2">
          <h3 class="text-lg font-header font-bold text-meal-gray-dark mb-4">Letzte Aktivitäten</h3>

          <div class="space-y-4">
            <div v-for="(activity, index) in recentActivities" :key="index"
                 class="flex items-start p-3 hover:bg-meal-light rounded-lg transition-colors duration-150">
              <div :class="`bg-${getActivityIconBg(activity.type)} p-2 rounded-lg mr-4`">
                <svg v-if="activity.type === 'meal'" xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 text-white"
              ...
              <div class="flex-grow">
                <div class="flex justify-between">
                  <h4 class="font-bold text-meal-gray-dark">{{ getActivityTitle(activity) }}</h4>
                  <span class="text-sm text-meal-gray">{{ formatDate(activity.date) }}</span>
                </div>
                <p class="text-meal-gray">{{ activity.description }}</p>

                <!-- Spezifische Darstellung je nach Typ -->
                <div v-if="activity.type === 'payment'" class="mt-1 flex items-center text-sm">
                  ...
                  <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 text-meal-gray mx-1" fill="none"
                       viewBox="0 0 24 24" stroke="currentColor">
                  ...
                  <span class="ml-auto font-bold text-meal-positive">
                    {{ formatCurrency(Number(activity.amount)) }}
                  </span>
                </div>

                <div v-if="activity.type === 'meal'" class="mt-1 flex items-center text-sm">
                  <div class="flex items-center">
                    <div class="w-6 h-6 rounded-full flex items-center justify-center text-white font-bold mr-1"
                  ...
                  <span class="ml-auto font-bold text-meal-primary">
                    {{ formatCurrency(Number(activity.amount)) }}
                  </span>
                </div>
              </div>
          ...
          <button @click="showMoreActivities"
                  class="w-full mt-4 py-2 text-meal-primary hover:text-meal-dark font-medium transition-colors duration-200">
        ...
        <!-- Top Schulden -->
        <div class="bg-white rounded-xl shadow-meal p-6">
          <h3 class="text-lg font-header font-bold text-meal-gray-dark mb-4">Top Schulden</h3>

          <div class="space-y-4">
            <div v-for="(debt, index) in topDebts" :key="index"
                 class="flex items-center justify-between p-3 border-b border-meal-gray-light">
              <div class="flex items-center">
                ...
                <span class="text-sm">schuldet</span>
              </div>
              <div class="flex items-center">
                <span class="font-bold text-meal-error mr-3">{{ formatCurrency(debt.amount) }}</span>
      ...
      <div class="bg-white rounded-xl shadow-meal p-6 mb-8">
        <h3 class="text-lg font-header font-bold text-meal-gray-dark mb-4">Schnellzugriff</h3>

        <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
          <button
              @click="goToMeal"
              class="flex items-center p-4 rounded-lg border border-meal-gray-light hover:bg-meal-light transition-colors duration-200"
          >
            <div class="p-2 bg-meal-primary rounded-full mr-3">
            ...
            <div class="text-left">
              <h4 class="font-bold text-meal-gray-dark">Neue Mahlzeit</h4>
              <p class="text-sm text-meal-gray">Mahlzeit erfassen</p>
            </div>
          </button>
          ...
          <button
              ...
              class="flex items-center p-4 rounded-lg border border-meal-gray-light hover:bg-meal-light transition-colors duration-200"
          >
            <div class="p-2 bg-meal-accent rounded-full mr-3">
              <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 text-white" fill="none" viewBox="0 0 24 24"
                   stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                      d="M17 9V7a2 2 0 00-2-2H5a2 2 0 00-2 2v6a2 2 0 002 2h2m2 4h10a2 2 0 002-2v-6a2 2 0 00-2-2H9a2 2 0 00-2 2v6a2 2 0 002 2zm7-5a2 2 0 11-4 0 2 2 0 014 0z"/>
            ...
            <div class="text-left">
              <h4 class="font-bold text-meal-gray-dark">Neue Zahlung</h4>
              <p class="text-sm text-meal-gray">Zahlung erfassen</p>
            </div>
          </button>
          ...
          <button
              ...
              class="flex items-center p-4 rounded-lg border border-meal-gray-light hover:bg-meal-light transition-colors duration-200"
          >
            <div class="p-2 bg-meal-secondary rounded-full mr-3">
              <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 text-white" fill="none" viewBox="0 0 24 24"
                   stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                      d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197M13 7a4 4 0 11-8 0 4 4 0 018 0z"/>
            ...
            <div class="text-left">
              <h4 class="font-bold text-meal-gray-dark">Benutzer</h4>
              <p class="text-sm text-meal-gray">Benutzer verwalten</p>
            </div>
          </button>
        </div>
      </div>
...
// Top Schulden
const topDebts = ref([
  { fromId: 1, fromName: 'Max Mustermann', toId: 3, toName: 'Jonas Schmidt', amount: 36.50 },
  { fromId: 2, fromName: 'Lisa Müller', toId: 4, toName: 'Sarah Weber', amount: 28.75 },
  { fromId: 3, fromName: 'Jonas Schmidt', toId: 2, toName: 'Lisa Müller', amount: 15.20 }
]);

// Ausgaben pro Person
const userExpenseData = ref([
  { id: 1, name: 'Max Mustermann', amount: 220.50, percentage: 35 },
  { id: 2, name: 'Lisa Müller', amount: 156.80, percentage: 25 },
  { id: 3, name: 'Jonas Schmidt', amount: 130.25, percentage: 21 },
  { id: 4, name: 'Sarah Weber', amount: 118.95, percentage: 19 }
]);
...
function showMoreActivities() {
  alert('Weitere Aktivitäten werden geladen...');
}
