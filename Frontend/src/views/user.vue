<template>
  <div class="min-h-screen flex flex-col flex-grow bg-meal-light font-sans pb-8">
    ...
    <header class="bg-meal-primary text-white p-4 shadow-md mb-4 sm:mb-8">
      <div class="container mx-auto flex items-center justify-between">
        <div class="flex items-center space-x-2">
          ...
          <h1 class="text-xl sm:text-2xl font-header font-bold">Benutzer</h1>
        </div>
        <button @click="goBackToHomeScreen"
                class="text-white hover:text-meal-accent-light transition-colors duration-200 text-sm sm:text-base">
    ...
    <div class="container mx-auto px-4">
      <div class="bg-white rounded-xl shadow-meal p-4 sm:p-6 mb-6 sm:mb-8">
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-4 sm:mb-6 gap-3 sm:gap-0">
          <h2 class="text-xl sm:text-2xl font-header font-bold text-meal-gray-dark">Benutzer verwalten</h2>
          <button
              @click="showAddForm = true"
              class="bg-meal-primary hover:bg-meal-dark text-white font-bold py-2 px-4 rounded-lg transition-colors duration-200 w-full sm:w-auto"
          >
        ...
        <!-- Desktop Benutzer Liste (versteckt auf mobilen Geräten) -->
        ...
        <!-- Mobile Benutzer Liste (Kartenansicht) -->
        <div class="sm:hidden">
          <div v-if="users.length === 0" class="py-4 px-4 text-center text-meal-gray">
            Keine Benutzer vorhanden. Füge deinen ersten Benutzer hinzu!
          </div>
            ...
            <div v-for="user in users" :key="user.id"
              ...
              <div class="flex items-center justify-between mb-3">
                <div class="flex items-center">
                  ...
                  <div>
                    <h3 class="font-bold text-meal-gray-dark">{{ user.name }}</h3>
                ...
                <div class="flex">
                  <button
                      @click="editUser(user)"
                      class="text-meal-primary hover:text-meal-dark mr-3 p-2"
                      title="Bearbeiten"
                  >
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
                      <path
      ...
      <div class="grid grid-cols-1 sm:grid-cols-3 gap-4 sm:gap-6 mb-6 sm:mb-8">
        <div class="bg-white rounded-xl shadow-meal p-4 sm:p-6">
          <h3 class="text-base sm:text-lg font-header font-bold text-meal-gray-dark mb-1 sm:mb-2">Gesamt Benutzer</h3>
          <p class="text-2xl sm:text-3xl font-bold text-meal-primary">{{ users.length }}</p>
        </div>
        ...
        <div class="bg-white rounded-xl shadow-meal p-4 sm:p-6">
          <h3 class="text-base sm:text-lg font-header font-bold text-meal-gray-dark mb-1 sm:mb-2">Ausstehende Zahlungen</h3>
          <p class="text-2xl sm:text-3xl font-bold text-meal-positive">{{ formatCurrency(totalCredits) }}</p>
        </div>
      </div>
    </div>
    ...
    <div v-if="showAddForm" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div class="bg-white rounded-xl shadow-xl p-4 sm:p-6 w-full max-w-md">
        <div class="flex justify-between items-center mb-4">
          <h3 class="text-lg sm:text-xl font-header font-bold text-meal-gray-dark">Neuen Benutzer hinzufügen</h3>
    ...
    <div v-if="showEditForm" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div class="bg-white rounded-xl shadow-xl p-4 sm:p-6 w-full max-w-md">
        <div class="flex justify-between items-center mb-4">
          <h3 class="text-lg sm:text-xl font-header font-bold text-meal-gray-dark">Benutzer bearbeiten</h3>
    ...
    <div v-if="showDeleteConfirm" class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div class="bg-white rounded-xl shadow-xl p-4 sm:p-6 w-full max-w-md">
        <h3 class="text-lg sm:text-xl font-header font-bold text-meal-gray-dark mb-4">Benutzer löschen</h3>
        <p class="mb-6 text-meal-gray-dark text-sm sm:text-base">
        ...
        <div class="flex justify-end space-x-3">
          <button
              ...
              class="px-4 py-2 border border-meal-gray-light rounded-lg text-meal-gray-dark hover:bg-meal-gray-light transition-colors duration-200 text-sm sm:text-base"
          >
            Abbrechen
          </button>
          <button
              @click="deleteUser"
              class="px-4 py-2 bg-meal-error hover:bg-red-700 text-white rounded-lg transition-colors duration-200 text-sm sm:text-base"
          >
            Löschen
          </button>
        </div>
